...
export class HttpService {
	...
	get(uri: string, params: any[] = []): Observable<any> {
		...
		return this._http
			.get<any>(this.cloeBEUrl + uri, { headers, params: _httpParams })
			.pipe(takeUntil(this.cancelHttpCall));
	}

	getDocument(uri: string): Observable<any> {
		return this._http.get(this.cloeBEUrl + uri, {
			responseType: 'blob',
		});
	}
	...
	post(uri: string, body: any = {}, params: any[] = []): Observable<any> {
		...
		return this._http
			.post<any>(this.cloeBEUrl + uri, body, { headers, params: _httpParams })
			.pipe(takeUntil(this.cancelHttpCall));
	}
	...
	put(uri: string, body: any = {}, params: any[] = []): Observable<any> {
		...
		return this._http
			.put<any>(this.cloeBEUrl + uri, body, { headers, params: _httpParams })
			.pipe(takeUntil(this.cancelHttpCall));
	}
